...
class BaseGateway():
    def __init__(self, data_paths: Tuple[str]=None, file_share_dir: str=_FILE_SHARE_DIR):
        self.client = jane.kaggle_evaluation.core.relay.Client('inference_server' if IS_RERUN else 'localhost')
    ...
    def validate_prediction_batch(
        self,
        prediction_batch: pl.DataFrame,
        sample_submission_batch: pl.DataFrame):
    ...
    def write_submission(self, predictions):
        ...
        if isinstance(predictions, list):
            ...
            if isinstance(predictions[0], pl.DataFrame):
                try:
